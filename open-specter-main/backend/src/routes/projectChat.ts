...
import { activityEventExists, logActivityEvent, titleFromPrompt } from "../lib/activity";
        ...
        if (createdChat) {
            ...
            if (!hasExistingActivity) {
                await logActivityEvent({
                    ...
                    userId,
                    eventType: "assistant_chat_created",
                    title: titleFromPrompt(lastUser.content, "New project chat"),
                    entityType: "chat",
                    entityId: chatId,
                    projectId,
                    metadata: {
            ...
            if (!hasExistingActivity) {
                await logActivityEvent({
                    ...
                    metadata: {
                        file_count: lastUser.files?.length ?? 0,
                        source: "first_message_existing_project_chat",
                    },
                });
            }
        }
