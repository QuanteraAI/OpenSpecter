...
import type { OpenSpecterDocument, OpenSpecterWorkflow } from "../shared/types";
import { createTabularReview } from "@/app/lib/openSpecterApi";
import { useRouter } from "next/navigation";
import { formatIcon, formatLabel } from "../tabular/columnFormat";
import { useDirectoryData } from "../shared/useDirectoryData";
import { FileDirectory } from "../shared/FileDirectory";
import type { OpenSpecterProject } from "../shared/types";
import { useChatHistoryContext } from "@/app/contexts/ChatHistoryContext";

interface Props {
    workflows: OpenSpecterWorkflow[];
    workflow: OpenSpecterWorkflow | null;
    onClose: () => void;
}
...
// ---------------------------------------------------------------------------
function AssistantPanel({ workflow }: { workflow: OpenSpecterWorkflow }) {
    return (
...
// ---------------------------------------------------------------------------
function TabularPanel({ workflow }: { workflow: OpenSpecterWorkflow }) {
        ...
        <div className="flex-1 border-l border-t border-gray-200 flex flex-col overflow-hidden px-3 pb-3">
            ...
            <div className="flex-1 overflow-y-auto border border-gray-200 rounded-md bg-gray-50">
                {columns.length === 0 ? (
                    ...
                    columns.map((col) => {
                        const isExpanded = expandedIndexes.has(col.index);
                        ...
                        return (
                            ...
                            >
                                <button
                                    ...
                                    data-testid={`workflow-prompt-block-trigger-${col.index}`}
                                ...
                                <AnimatePresence initial={false}>
                                    {isExpanded && (
                                        <motion.div
                                            key={`workflow-prompt-block-${col.index}`}
                                            initial={{ height: 0, opacity: 0 }}
                                            animate={{ height: "auto", opacity: 1 }}
                                            exit={{ height: 0, opacity: 0 }}
                                            transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}
                                            className="overflow-hidden"
                                            data-testid={`workflow-prompt-block-content-${col.index}`}
                                        >
                                            <div className="space-y-3 border-t border-gray-200 bg-white px-4 py-3 text-sm leading-relaxed text-gray-600 font-serif">
                                                {col.tags && col.tags.length > 0 && (
                                                    <div>
                                                        <p className="mb-1.5 font-sans text-xs font-medium text-gray-400">
                                                            Tags
                                                        </p>
                                                ...
                                                <div>
                                                    <p className="mb-1 font-sans text-xs font-medium text-gray-400">
                                                        Prompt
                                                    </p>
                                                    <MarkdownBody
                                                        content={
...
export function DisplayWorkflowModal({ workflows, workflow, onClose }: Props) {
    ...
    const [selected, setSelected] = useState<OpenSpecterWorkflow | null>(workflow);
    const [listSearch, setListSearch] = useState("");
    const [typeFilter, setTypeFilter] = useState<"all" | "tabular" | "assistant">("all");
    ...
    async function handleStartChat() {
        ...
        try {
            ...
            if (!chatId) return;
            const allDocs: OpenSpecterDocument[] = [
                ...standaloneDocuments,
    ...
    return createPortal(
        <div className="fixed inset-0 z-[101] flex items-center justify-center bg-black/20 backdrop-blur-xs">
            <div
                data-testid="select-workflow-modal"
                className={`w-[calc(100vw-48px)] rounded-2xl bg-white shadow-2xl flex flex-col h-[760px] max-h-[calc(100dvh-48px)] transition-[max-width] duration-200 ${screen === "select" ? "max-w-6xl" : "max-w-4xl"}`}
            >
                {/* Header */}
                        ...
                        <div className="flex flex-row flex-1 min-h-0 overflow-hidden">
                            ...
                            <div className="w-80 shrink-0 flex flex-col border-t border-gray-200">
                                ...
                                <div
                                    ...
                                    data-testid="workflow-modal-type-filter"
                                    className="px-3 py-2 shrink-0 border-b border-gray-100 flex items-center gap-1"
                                >
                                    {(
                                        [
                                            { id: "all" as const, label: "All", count: workflows.length },
                                            { id: "tabular" as const, label: "Tabular", count: workflows.filter((w) => w.type === "tabular").length },
                                            { id: "assistant" as const, label: "Assistant", count: workflows.filter((w) => w.type === "assistant").length },
                                        ]
                                    ).map((opt) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                data-testid={`workflow-modal-type-filter-${opt.id}`}
                                                onClick={() => setTypeFilter(opt.id)}
                                                className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-[520] font-space-grotesk tracking-[-0.005em] transition-[background-color,color,border-color] duration-150 ${
                                                    isActive
                                ...
                                <div className="overflow-y-auto flex-1">
                                    {workflows
                                        .filter((wfItem) => typeFilter === "all" || wfItem.type === typeFilter)
