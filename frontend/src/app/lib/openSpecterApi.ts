/**
 * Open Specter API client — all requests to the Node.js backend.
...
import type {
    AssistantEvent,
    OpenSpecterChat,
    OpenSpecterChatDetailOut,
    OpenSpecterCitationAnnotation,
    OpenSpecterDocument,
    OpenSpecterFolder,
    OpenSpecterMessage,
    OpenSpecterProject,
...
interface ServerMessage {
    ...
    annotations?: OpenSpecterCitationAnnotation[] | null;
    created_at: string;
}
interface ServerChatDetailOut {
    chat: OpenSpecterChat;
...
export interface RecentActivityEvent {
    ...
    entity_type: "chat" | "tabular_review" | "workflow" | string | null;
...
export async function listRecentActivity(
    ...
    return apiRequest<RecentActivityEvent[]>(`/activity/recent?limit=${limit}`);
}
...
export async function listProjects(): Promise<OpenSpecterProject[]> {
    return apiRequest<OpenSpecterProject[]>("/projects");
}
...
export async function getProject(projectId: string): Promise<OpenSpecterProject> {
    return apiRequest<OpenSpecterProject>(`/projects/${projectId}`);
}
...
): Promise<OpenSpecterFolder> {
    return apiRequest<OpenSpecterFolder>(`/projects/${projectId}/folders`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            name,
...
export async function renameProjectFolder(
    ...
    return apiRequest<OpenSpecterFolder>(
...
): Promise<OpenSpecterDocument> {
    return apiRequest<OpenSpecterDocument>(
...
export interface OpenSpecterDocumentVersion {
    id: string;
...
export async function listDocumentVersions(
    ...
    versions: OpenSpecterDocumentVersion[];
}> {
...
export async function uploadDocumentVersion(
    ...
    file: File,
    displayName?: string,
): Promise<OpenSpecterDocumentVersion> {
    const authHeaders = await getAuthHeader();
    ...
    return response.json() as Promise<OpenSpecterDocumentVersion>;
}
...
export async function renameDocumentVersion(
    ...
    return apiRequest<OpenSpecterDocumentVersion>(
...
export async function uploadProjectDocument(
    ...
    if (!response.ok) throw new Error(await response.text());
    return response.json() as Promise<OpenSpecterDocument>;
}
...
export async function listStandaloneDocuments(): Promise<OpenSpecterDocument[]> {
    return apiRequest<OpenSpecterDocument[]>("/single-documents");
}
...
export async function listChats(): Promise<OpenSpecterChat[]> {
    return apiRequest<OpenSpecterChat[]>("/chat");
}

export async function listProjectChats(projectId: string): Promise<OpenSpecterChat[]> {
    return apiRequest<OpenSpecterChat[]>(`/projects/${projectId}/chats`);
}

export async function getChat(chatId: string): Promise<OpenSpecterChatDetailOut> {
    const raw = await apiRequest<ServerChatDetailOut>(`/chat/${chatId}`);
    const messages: OpenSpecterMessage[] = raw.messages.map((m) => {
        if (m.role === "user") {
            return {
                role: "user",
...
export async function generateChatTitle(
    ...
    // Strip the LegalDataHunter research-context block (if present) before
    // titling so the chat name reflects the user's actual question.
    const start = message.indexOf("[[LDH:CTX:START]]");
    const end = message.indexOf("[[LDH:CTX:END]]");
    let cleaned = message;
    if (start >= 0 && end > start) {
        cleaned = (
            message.slice(0, start) +
            message.slice(end + "[[LDH:CTX:END]]".length)
        )
            .replace(/^[\s-]+|[\s-]+$/g, "")
            .trim();
    }
    return apiRequest<{ title: string }>(`/chat/${chatId}/generate-title`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: cleaned || message }),
    });
}
...
type WorkflowType = OpenSpecterWorkflow["type"];

export async function listWorkflows(
    type: WorkflowType,
): Promise<OpenSpecterWorkflow[]> {
    return apiRequest<OpenSpecterWorkflow[]>(`/workflows?type=${type}`);
}

export async function getWorkflow(workflowId: string): Promise<OpenSpecterWorkflow> {
    return apiRequest<OpenSpecterWorkflow>(`/workflows/${workflowId}`);
}
...
export async function createWorkflow(payload: {
    ...
    return apiRequest<OpenSpecterWorkflow>("/workflows", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
    });
